(function($, win) {
	$.fn.wrpage = function(options) {
		...
		function init(size, cur) {
			...
			//中间
			if (cur < 5) {
				for (var i = 1; i <= 6&&i<=size; i++) {
					if (i == cur) {
						_html += '<span class="curwrpage wrpage wrpage_number">' + cur + '</span>'
					} else {
						_html += '<a href='+"${pageContext.request.contextPath}/productlist?currentPage="+i+"&pageSize=${page.pageSize}&cid=${page.cid}"+' class="wrpage wrpage_number">' + i + '</a>'
					}
				}
			} else if (cur >= 5) {
